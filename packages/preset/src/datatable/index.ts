import type { ColumnContext, ColumnProps } from "primevue/column"
import type { ColumnGroupContext, ColumnGroupProps } from "primevue/columngroup"
import type { DataTableContext, DataTableProps, DataTableState } from "primevue/datatable"
import type { RowContext, RowProps } from "primevue/row"

export interface IDataTable{
    props: DataTableProps
    context: DataTableContext
    state: DataTableState
    contextColumnGroup: ColumnGroupContext
    instance: any
}
...
export default {
    ...
    tbody: {
        class: "border-t border-surface-300 dark:border-surface-600"
    },
    tfoot: ({ context }: any) => ({
        class: [
            {
    ...
    column: {
        headercell: ({ context, props }: IColumn) => ({
            class: [
                "font-semibold",
                "text-xs",

                // Position
                { "sticky z-20 border-b": props.frozen || props.frozen === null },
                ...
                context?.size === "small"
                    ...
                    : "py-3.5 px-3",
                // Color
                (props.sortable === null || props.sortable) && context.sorted
                    ? "text-primary-500"
                    : "bg-surface-400/10 text-surface-700",
                (props.sortable === null || props.sortable) && context.sorted
                    ? "dark:text-primary-400"
                    : "dark:text-white/80 dark:bg-surface-600",
                "border-surface-200 dark:border-surface-700 ",
                ...
                {
                    "transition duration-200":
                        props.sortable === null || props.sortable,
                },

                // Misc
                { "cursor-pointer": props.sortable === null || props.sortable },
                {
        ...
        sort: ({ context }: IColumn) => ({
            class: [
                context.sorted ? "text-primary-500" : "text-surface-700",
                context.sorted ? "dark:text-primary-400" : "dark:text-white/80",
            ],
        }),
        bodycell: ({ props, context, state }: IColumn) => ({
            class: [
                //Position
                // { "sticky box-border border-b": parent.instance.frozenRow },  ==> Verify
                {
                    "sticky box-border border-b":
                        props.frozen || props.frozen === null,
                },
                ...
                {
                    "bg-surface-0 dark:bg-surface-800":
                        // parent.instance.frozenRow || ===> Verify
                        props.frozen || props.frozen === null,
                },
        ...
        filtermenubutton: ({ context }: IColumn) => ({
            class: [
                "relative",
                // Flex & Alignment
        ...
        headerfilterclearbutton: ({ context }: IColumn) => ({
            class: [
                "relative",
    ...
    bodyrow: ({ context, props }: IDataTable) => ({
        class: [
            // Color
            "dark:text-white/80",
            {
            ...
            // { "bg-surface-0 dark:bg-surface-800": props.frozenRow }, ===> Verify
            {
